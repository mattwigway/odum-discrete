...
- Install necessary Python packages using `pip`: `larch6`, `statsmodels`, `pandas`, `numpy`, `scipy`
...
```{python}
#| filename: logistic_black_hispanic.ipynb
#| output: false
#| echo: true
import pandas as pd
import numpy as np, statsmodels.api as sm
...
- We're going to use the [larch](https://larch.driftless.xyz) package to do so

## Estimation code

```{python}
#| filename: larch_binary.ipynb
#| output: false
import pandas as pd
import numpy as np
import larch as lx
...
```{python}
#| filename: larch_binary_race.ipynb
import pandas as pd
import numpy as np
import larch as lx
from larch import P, X
...
# utility of True (expecting to work from home)
m.utility_co[2] = P.intercept + P.age * X.age + P.college * X.college + P.male * X.gender_Male + P.black * X.black
...
# create our choice variable - it is coded as text in the data but `larch` requires
# it to be numeric
...
data = lx.Dataset.construct.from_idco(pd.get_dummies(df), alts={
    1: "Car",
    2: "Transit",
    3: "Walk",
    4: "Bike"
...
m.utility_co[2] = P.asc_transit + P.income_over_100k_transit * X.income_over_100k
m.utility_co[3] = P.asc_walk + P.income_over_100k_walk * X.income_over_100k
m.utility_co[4] = P.asc_bike + P.income_over_100k_bike * X.income_over_100k

m.choice_co_code = "numeric_mode"


m.maximize_loglike(stderr=True)
```
...
```{python}
#| filename: seattle_mnl_income.ipynb
import pandas as pd
import numpy as np
import larch as lx
from larch import P, X
...
```{python}
#| filename: seattle_mnl_income_rush.ipynb
import pandas as pd
import numpy as np
import larch as lx
from larch import P, X
...
m.utility_co[2] = (
    P.asc_transit +
    P.income_over_100k_transit * X.income_over_100k +
    P.rush_transit * X.rush_hour
)
m.utility_co[3] = (
    P.asc_walk +
    P.income_over_100k_walk * X.income_over_100k +
    P.rush_walk * X.rush_hour
)
m.utility_co[4] = (
    P.asc_bike +
    P.income_over_100k_bike * X.income_over_100k +
    P.rush_bike * X.rush_hour
)
...
```{python}
#| filename: seattle_mnl_availability.ipynb
import pandas as pd
import numpy as np
import larch as lx
from larch import P, X
...
```{python}
#| filename: seattle_mnl_availability_fixed.ipynb
import pandas as pd
import numpy as np
import larch as lx
from larch import P, X
...
## Adding a generic variable in `larch`

```{python}
#| filename: seattle_mnl_generic.ipynb
import pandas as pd
import numpy as np
import larch as lx
from larch import P, X
...
m.utility_co[1] = P.travel_time * X.car_travel_time_mins

m.utility_co[2] = (
    P.asc_transit +
    P.income_over_100k_transit * X.income_over_100k +
    P.rush_transit * X.rush_hour +
    P.travel_time * X.transit_travel_time_mins
)
...
m.utility_co[3] = (
    ...
    P.income_over_100k_walk * X.income_over_100k +
    P.rush_walk * X.rush_hour +
    P.travel_time * X.walk_travel_time_mins
)
m.utility_co[4] = (
    P.asc_bike +
    P.income_over_100k_bike * X.income_over_100k +
    P.rush_bike * X.rush_hour +
    P.travel_time * X.bike_travel_time_mins
)
...
```{python}
#| filename: seattle_mnl_generic_cost.ipynb
import pandas as pd
import numpy as np
import larch as lx
from larch import P, X
...
m.utility_co[1] = P.travel_time * X.car_travel_time_mins + P.travel_cost * X.car_travel_cost
...
m.utility_co[2] = (
    ...
    P.rush_transit * X.rush_hour +
    P.travel_time * X.transit_travel_time_mins +
    P.travel_cost * X.transit_travel_cost
)
...
m.utility_co[3] = (
    ...
    P.rush_walk * X.rush_hour +
    P.travel_time * X.walk_travel_time_mins +
    P.travel_cost * X.walk_travel_cost
)
m.utility_co[4] = (
    P.asc_bike +
    P.income_over_100k_bike * X.income_over_100k +
    P.rush_bike * X.rush_hour +
    P.travel_time * X.bike_travel_time_mins +
    P.travel_cost * X.bike_travel_cost
)
...
data_road_cost = lx.Dataset.construct.from_idco(pd.get_dummies(df_road_cost), alts={
    1: "Car",
    2: "Transit",
    3: "Walk",
    4: "Bike"
...
data_transit_time = lx.Dataset.construct.from_idco(pd.get_dummies(df_transit_time), alts={
    1: "Car",
    2: "Transit",
    3: "Walk",
    4: "Bike"
...
- We see the log-likelihood in the `larch` output
...
## Calculating pseudo-$R^2$ in `larch`

- `larch` automatically calculates the log-likelihood at zero
...
- Unfortunately, `larch` does not calculate this automatically
- It is pretty easy to calculate though, by removing everything but the ASCs
- Exercise: go ahead and do this now

## Calculating pseudo-$R^2$ in `larch`: results

```{python}
#| filename: seattle_constants.ipynb
import pandas as pd
import numpy as np
import larch as lx
from larch import P, X
...
m.utility_co[2] = P.asc_transit

m.utility_co[3] = P.asc_walk

m.utility_co[4] = P.asc_bike
...
- `larch` can work with either wide data (which it refers to as `idco`) or long data (`idca`), or a combination of both
...
## Estimating a nested logit model in `larch`

- We will put transit and driving together in a "motorized" nest.

## Estimating a nested logit model in `larch`: code

```{python}
#| filename: seattle_nested.ipynb
import pandas as pd
import numpy as np
import larch as lx
from larch import P, X
...
# Here we define the nesting structure, combining transit and driving into motorized
m.graph.new_node(parameter="motorized", children=[1, 2], name="Motorized")
...
- Our inclusive-value parameter is 1, which means our nested logit model is the same as an MNL
- However, `larch` by default constrains exclusive value parameters to $0.001 \leq \theta \leq 1$
- If we add `m.pmaximum[:] = np.inf` before `m.maximize_loglike`, we will get an inclusive-value parameter of 1.32
...
```{python}
#| filename: seattle_nested_active.ipynb
import pandas as pd
import numpy as np
import larch as lx
from larch import P, X
...
# Here we define the nesting structure, combining transit and driving into motorized
m.graph.new_node(parameter="active", children=[3, 4], name="Active")
...
- There are likely unobserved shared attributes between wlaking and bicycling that affect the choice
:::
...
- 0.89 isn't that different from 1
- How do we know we need a nested logit model?
- When there's just one nest we can use a standard $z$-test and $p$-value
    - We need to test that the nesting parameter ($\theta$ or $\mu$) is different from one, not zero like most coefficients
    - `larch` does this correctly, but many other programs do not
...
- If the model did _not_ converge, it means `larch` did not find a solution
...
- We estimated models using the `larch` package, other packages are Biogeme (Python) and apollo (R)
