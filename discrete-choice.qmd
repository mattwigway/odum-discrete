...
- Install necessary Python packages using `pip`: `biogeme`, `statsmodels`, `pandas`, `numpy`, `statsmodels`
...
Stored in "wfh_logit.html", or some variation—look for most recent.
...
model = bio.BIOGEME(data, logprob)
model.modelName = "seattle_mode_choice"
model.calculateNullLoglikelihood(av)
result = model.estimate()
...
```{python}
#| echo: false
params = result.getEstimatedParameters()
Markdown(
    tabulate.tabulate(
...
## Interpreting the coefficient changes
...
More detail: see Section 5.2 of @ben-akiva_discrete_1985
...
```{python}
choice_probabilities.apply(np.mean)
```
...
- There's a lot more that goes into designing good choice experiments; see @bliemer_designing_2024
...
- For instance, you could estimated this model
